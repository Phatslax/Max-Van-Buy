...
\}\
\
st.set_page_config(page_title="Ford Transit Custom Depreciation Tool", page_icon="
\f1 \uc0\u55357 \u56976 
\f0 ", layout="centered")\
st.title("
\f1 \uc0\u55357 \u56976 
\f0  Ford Transit Custom Depreciation Tool (UK)")\
...
price = col3.number_input("Advert Price (\'a3)", min_value=0, value=12000, step=500)\
\
...
def extract_data(url):\
    ...
    try:\
        ...
        price_match = re.search(r'\'a3([\\d,]+)', page)\
...
\f0  Summary")\
    ...
    st.write(f"**Advert price:** \'a3\{price:,.0f\}")\
    st.write(f"**Mileage:** \{mileage:,\} miles")\
    st.write(f"**Repair status:** \{repair_status\}")\
    st.write(f"**Expected value (adjusted):** \'a3\{expected_adjusted:,.0f\}")\
    st.write(f"**Deviation from expected:** \{deviation_pct:+.1f\}% (\{rating\})")\
    st.write(f"**Average monthly depreciation:** \'a3\{monthly_dep:,.0f\}/month")\
\
...
\
    ...
    story.append(Paragraph(f"<b>Advert Price:</b> \'a3\{price:,.0f\}", styles['Normal']))\
    story.append(Paragraph(f"<b>Expected Value (adjusted):</b> \'a3\{expected_adjusted:,.0f\}", styles['Normal']))\
    story.append(Paragraph(f"<b>Deviation from Expected:</b> \{deviation_pct:+.1f\}% (\{rating\})", styles['Normal']))\
    story.append(Paragraph(f"<b>Average Monthly Depreciation:</b> \'a3\{monthly_dep:,.0f\}/month", styles['Normal']))\
    story.append(Spacer(1, 12))\
\
    chart_buf.seek(0)\
